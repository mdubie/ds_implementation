...
(function() {
  ...
  describe('singly linked lists', function() {
    ...
    describe('LinkedList constructor functions', function() {
      it("should create lists", function() {
        var list = new LinkedList();
        expect(typeof(list)).to.deep.eql('object'); 
        expect(list.length).to.deep.eql(0);
        expect(list.head).to.deep.eql(null);
      });
    });

    describe('Node constructor functions', function() {
      it("should create nodes", function() {
        var node1 = new Node('apple');
        var node2 = new Node('bananas');
        node1.next = node2
        expect(typeof(node1)).to.deep.eql('object'); 
        expect(node1.value).to.deep.eql('apple');
        expect(node1.next).to.deep.eql({value: 'bananas', next: null});
        expect(node1.length).to.deep.eql(undefined);
      });
    });
